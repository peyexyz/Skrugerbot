...
from Telegram import telethn as skruger, client
from Telegram.status import *



@skruger.on(events.NewMessage(pattern="^[!?/]join ?(.*)"))
@skruger.on(events.NewMessage(pattern="^[!?/]userbotjoin ?(.*)"))
@is_admin
async def _(e, perm):
    chat_id = e.chat_id
...
@skruger.on(events.NewMessage(pattern="^[!?/]pjoin ?(.*)"))
@is_admin        
async def _(e, perm):
    chat_id = e.chat_id
